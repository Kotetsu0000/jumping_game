...
let p5playInit = function () {
    ...
    const fixRound = (val, slop) =>
        Math.abs(val - Math.round(val)) <= (slop || linearSlop)
            ? Math.round(val)
            : val;
    ...
    this.P5Play = class {
        ...
        constructor() {
            ...
            if (typeof navigator == 'object') {
                ...
                if (idx > -1) {
                    let version = navigator.userAgent.substring(
                        idx + 10,
                        idx + 12
                    );
    ...
    this.Sprite = class {
        ...
        constructor(x, y, w, h, physicsType) {
            ...
            Object.defineProperty(this._pos, 'x', {
                get() {
                    ...
                    let x =
                        (_this.body.getPosition().x / _this.tileSize) *
                        $.world.meterSize;
                ...
                set(val) {
                    ...
                    if (_this.body) {
                        let pos = new pl.Vec2(
                            (val * _this.tileSize) / $.world.meterSize,
                            _this.body.getPosition().y
                        );
            ...
            Object.defineProperty(this._pos, 'y', {
                get() {
                    ...
                    let y =
                        (_this.body.getPosition().y / _this.tileSize) *
                        $.world.meterSize;
            ...
            Object.defineProperties(this._vel, {
                x: {
                    ...
                    set(val) {
                        val *= _this.tileSize;
                        if (_this.body) {
                            _this.body.setLinearVelocity(
                                new pl.Vec2(
                                    val,
                                    _this.body.getLinearVelocity().y
                                )
                            );
                        } else {
                ...
                y: {
                    ...
                    set(val) {
                        ...
                        if (_this.body) {
                            _this.body.setLinearVelocity(
                                new pl.Vec2(
                                    _this.body.getLinearVelocity().x,
                                    val
                                )
                            );
                        } else {
            ...
            for (let i = 0; i < this.groups.length; i++) {
                ...
                for (let prop of props) {
                    if (
                        !isNaN(prop) ||
                        prop[0] == '_' ||
                        skipProps.includes(prop) ||
                        $.Sprite.propsAll.includes(prop)
                    ) {
                        continue;
                    }
                    let val = g[prop];
            ...
            this._textSize ??=
                this.tileSize == 1 ? ($.canvas ? $.textSize() : 12) : 0.8;
        }
        ...
        addCollider(offsetX, offsetY, w, h) {
            if (this._removed) {
                console.error(
                    "Can't add colliders to a sprite that was removed."
                );
                return;
            }
            if (this.__collider == 3) {
        ...
        addSensor(offsetX, offsetY, w, h) {
            if (this._removed) {
                console.error(
                    "Can't add sensors to a sprite that was removed."
                );
                return;
            }
        ...
        _parseShape(offsetX, offsetY, w, h) {
            ...
            if (shape == 'box') {
                s = pl.Box(
                    dimensions.x / 2,
                    dimensions.y / 2,
                    scaleTo(offsetX, offsetY, this.tileSize),
                    0
                );
                ...
                if (
                    isSlop(
                        Math.abs(vecs[0].x) - Math.abs(vecs[vecs.length - 1].x)
                    ) &&
                    isSlop(
                        Math.abs(vecs[0].y) - Math.abs(vecs[vecs.length - 1].y)
                    )
                ) {
                ...
                if (this._originMode == 'start') {
                    ...
                    for (let i = 0; i < vecs.length; i++) {
                        let vec = vecs[i];
                        vecs[i] = scaleTo(
                            vec.x + offsetX - centerX,
                            vec.y + offsetY - centerY,
                            this.tileSize
                        );
                    }
                }
        ...
        set collider(val) {
            ...
            if (
                val == 'none' &&
                (this._shape == 'chain' || this._shape == 'polygon')
            ) {
                console.error(
            ...
            if (this._removed) {
                throw new Error(
                    'Cannot change the collider type of a sprite that was removed.'
                );
            }
        ...
        _getVertices(internalUse) {
            ...
            for (let i = 0; i < v.length; i++) {
                let arr = [
                    (v[i].x / this.tileSize) * $.world.meterSize + x,
                    (v[i].y / this.tileSize) * $.world.meterSize + y,
                ];
        ...
        set pos(val) {
            if (this.body) {
                let pos = new pl.Vec2(
                    (val.x * this.tileSize) / $.world.meterSize,
        ...
        ___step() {
            ...
            for (let event in eventTypes) {
                for (let k in this[event]) {
                    ...
                    shouldOverlap =
                        a._hasOverlap[b._uid] ?? b._hasOverlap[a._uid];
                    if (
                        (checkCollisions && shouldOverlap !== false) ||
                        (!checkCollisions && shouldOverlap !== true)
                    ) {
                        continue;
                    }
            ...
            if (this._removed) {
                if (
                    Object.keys(this._collisions).length == 0 &&
                    Object.keys(this._overlappers).length == 0
                ) {
                    if (this._isSprite) delete $.p5play.sprites[this._uid];
                    else if (!$.p5play.storeRemovedGroupRefs)
                        delete $.p5play.groups[this._uid];
        ...
        __draw() {
            if (!$.p5play.disableImages) {
                if (this._ani) {
                    ...
                    if (shouldScale) {
                        $.push();
                        $.scale(
                            this._scale._x * img.scale.x,
                            this._scale._y * img.scale.y
                        );
                    }
                    $.image(
                        img,
                        this._offset._x + img.offset.x,
                        this._offset._y + img.offset.y
                    );
                    if (shouldScale) $.pop();
                }
            }
            ...
            if (this.text !== undefined) {
                ...
                $.fill(this._textFill);
                if (this._textStrokeWeight)
                    $.strokeWeight(this._textStrokeWeight);
        ...
        move(distance, direction, speed) {
            ...
            if (
                ...
                x =
                    Math.round((this.x + Math.round(x)) / $.p5play.gridSize) *
                    $.p5play.gridSize;
                y =
                    Math.round((this.y + Math.round(y)) / $.p5play.gridSize) *
                    $.p5play.gridSize;
            } else {
                x += this.x;
                y += this.y;
            ...
            return (async () => {
                ...
                do {
                    ...
                    if (
                        ...
                        (Math.abs(this.vel.x) <= velThresh &&
                            Math.abs(this.vel.y) <= velThresh)
                    ) {
                        return false;
                    }
        ...
        rotate(angle, speed) {
            ...
            return (async () => {
                ...
                do {
                    ...
                    if (
                        (cw && this.rotationSpeed < slop) ||
                        (!cw && this.rotationSpeed > -slop)
                    ) {
                        return false;
                    }
                } while (
                    ((cw && ang > this.rotation) ||
                        (!cw && ang < this.rotation)) &&
                    slop < Math.abs(ang - this.rotation)
                );
        ...
        collides(target, callback) {
            ...
            return (
                this._collisions[target._uid] == 1 ||
                this._collisions[target._uid] <= -3
            );
        }
        ...
        overlaps(target, callback) {
            ...
            return (
                this._overlappers[target._uid] == 1 ||
                this._overlappers[target._uid] <= -3
            );
        }
        ...
        __removeContactsWith(o) {
            ...
            for (let ce = this.body.getContactList(); ce; ce = ce.next) {
                let c = ce.contact;
                if (
                    c.m_fixtureA.m_body.sprite._uid == o._uid ||
                    c.m_fixtureB.m_body.sprite._uid == o._uid
                ) {
    ...
    this.Ani = class extends Array {
        ...
        constructor() {
            ...
            if (
                ...
                (args[0]._isSprite || args[0]._isGroup)
            ) {
                owner = args[0];
                args = args.slice(1);
            ...
            if (
                typeof args[0] == 'string' &&
                (args[0].length == 1 || !args[0].includes('.'))
            ) {
                this.name = args[0];
                args = args.slice(1);
            }
             ...
             */
            this.offset = {
                x: owner.anis.offset.x ?? 0,
                y: owner.anis.offset.y ?? 0,
            };
            ...
            if (
                ...
                if (to && prefix1 != prefix2) {
                    ...
                    let num1 = parseInt(
                        from.slice(extIndex - digits1, extIndex),
                        10
                    );
                    num2 ??= parseInt(
                        to.slice(extIndex - digits2, extIndex),
                        10
                    );
                ...
                function _generateSheetFrames() {
                    ...
                    if (!w || !h) {
                        ...
                        if (!owner._dimensionsUndef && owner.w && owner.h) {
                            ...
                            if (
                                _this.spriteSheet.width <
                                _this.spriteSheet.height
                            ) {
        ...
        update() {
            ...
            if (this._cycles % this.frameDelay == 0) {
                ...
                if (
                    (this.targetFrame == -1 && this._frame == this.lastFrame) ||
    ...
    this.Group = class extends Array {
        ...
        push(...sprites) {
            ...
            for (let s of sprites) {
                if (!(s instanceof $.Sprite)) {
                    throw new Error(
                        'You can only add sprites to a group, not ' + typeof s
                    );
                }
                if (s.removed) {
    ...
    this.World = class extends planck.World {
        ...
        getSpritesAt(x, y, group, cameraActiveWhenDrawn = true) {
            ...
            this.queryAABB(aabb, (fxt) => {
                ...
                if (
                    fxt
                        .getShape()
                        .testPoint(fxt.getBody().getTransform(), point)
                ) {
                    fxts.push(fxt);
                }
                return true;
        ...
        getMouseSprites() {
            ...
            if ($.camera._wasOff) {
                let uiSprites = this.getSpritesAt(
                    $.mouse.canvasPos.x,
                    $.mouse.canvasPos.y,
                    $.allSprites,
                    false
        ...
        _endContact(contact) {
            ...
            for (let g of b.groups) {
                ...
                if (!inContact) {
                    g[contactType][a._uid] =
                        g[contactType][a._uid] != 0 ? -2 : -4;
                    a[contactType][g._uid] =
                        a[contactType][g._uid] != 0 ? -2 : -4;
                }
            }
            ...
            for (let g of a.groups) {
                ...
                if (!inContact) {
                    g[contactType][b._uid] =
                        g[contactType][b._uid] != 0 ? -2 : -4;
                    b[contactType][g._uid] =
                        b[contactType][g._uid] != 0 ? -2 : -4;
                    for (let g2 of b.groups) {
                        g[contactType][g2._uid] =
                            g[contactType][g2._uid] != 0 ? -2 : -4;
                        g2[contactType][g._uid] =
                            g2[contactType][g._uid] != 0 ? -2 : -4;
                    }
                }
            }
        }
        ...
        rayCastAll(startPos, direction, maxDistance, limiter) {
            ...
            if (typeof arguments[1] == 'number') {
                    ...
                    startPos.x + maxDistance * $.cos(direction),
                    startPos.y + maxDistance * $.sin(direction),
                    ts
                );
            } else {
    ...
    this.Camera = class {
        ...
        moveTo(x, y, speed) {
            ...
            if (speed <= 0) {
                console.warn(
                    'camera.moveTo: speed should be a positive number'
                );
                return Promise.resolve(false);
            }
            let a = y - this.y;
                ...
                for (let l of ['A', 'B']) {
                    ...
                    for (let axis of ['x', 'y']) {
                        Object.defineProperty(this[prop], axis, {
                            get() {
                                let val =
                                    (_this._j['m_localAnchor' + l][axis] /
                                        _this['sprite' + l].tileSize) *
                                    $.world.meterSize;
                                return $.p5play.friendlyRounding
                                    ? fixRound(val)
                                    : val;
                            },
                            set(val) {
                                ...
                                if (
                                    ...
                                    _this._j.m_length = pl.Vec2.distance(
                                        _this._j.m_bodyA.getWorldPoint(
                                            _this._j.m_localAnchorA
                                        ),
                                        _this._j.m_bodyB.getWorldPoint(
                                            _this._j.m_localAnchorB
                                        )
                                    );
                                } else if (
                                    _this.type == 'hinge' ||
                                    ...
                                    _this._j['m_localAnchor' + o][axis] =
                                        _this._j['m_body' + o].getLocalPoint(
                                            _this._j[
                                                'm_body' + l
                                            ].getWorldPoint(
                                                _this._j['m_localAnchor' + l]
                                            )
                                        )[axis];
                                }
                            },
    ...
    this.WheelJoint = class extends $.Joint {
        ...
        _display() {
            ...
            if (!this.offsetB.x && !this.offsetB.y) {
                ...
                let ancB = this.spriteB.body.getWorldPoint(
                    this._j.m_localAnchorB
                );
                ancB = scaleFrom(ancB.x, ancB.y, this.spriteB.tileSize);
                xB = ancB.x;
                yB = ancB.y;
        ...
        set angle(val) {
            ...
            this._j.m_localYAxisA = pl.Vec2.crossNumVec2(
                1.0,
    ...
    this.HingeJoint = class extends $.Joint {
        ...
        _display() {
            ...
            const rotatedOffsetAx =
                offsetAx * $.cos(rotationA) - offsetAy * $.sin(rotationA);
            const rotatedOffsetAy =
                offsetAx * $.sin(rotationA) + offsetAy * $.cos(rotationA);

            this._draw(
                this.spriteA.x + rotatedOffsetAx,
                this.spriteA.y + rotatedOffsetAy
            );
            this.visible = null;
        }
    ...
    function getRegularPolygon(lineLength, name) {
        ...
        if (l == lineLength)
            throw new Error('Invalid, not a regular polygon: ' + name);
        return l;
    }
...
main {
    ...
    class FriendlyError extends Error {
        constructor(func, errorNum, e) {
            ...
            if (m.base) msg = m.base + ln;
                ...
                msg =
                    errMsgs.generic[
                        Math.floor(Math.random() * errMsgs.generic.length)
                    ] + ln;
    ...
    this.Contro = class extends $.InputDevice {
        ...
        constructor(gp) {
            ...
            this._axeTriggers =
                this.gamepad.axes &&
                this.gamepad.axes[this.axeMapping.leftTrigger] !== undefined;
        ...
        _update() {
            ...
            if (this._axeTriggers) {
                ...
                if (
                    this.hasAnalogTriggers === undefined &&
...
let p5playPostDraw = function () {
    ...
    if ($.world.mouseTracking && $.mouse.isActive) {
        ...
        let isDragging =
            m.drag.left > 0 || m.drag.center > 0 || m.drag.right > 0;
    ...
    if (!$._q5) {
        ...
        $.p5play._fps =
            Math.round(
                1000 /
                    ($.p5play._postDrawFrameTime - $.p5play._preDrawFrameTime)
            ) || 1;
    }
...
if (p5.prototype?.registerMethod == undefined) {
    // p5.js v2
    console.error(
        'p5play is not compatible with p5.js v2. Please use p5.js v1 or q5.js. https://q5js.org'
    );
    ...
    p5.registerAddon((p5, proto, lifecycles) => {
        ...
        let methods = {
            ...
            loadStrings: () => [],
            loadFont: (pInst) =>
                new p5.Font(pInst, new FontFace('default', 'default.woff')),
        };
